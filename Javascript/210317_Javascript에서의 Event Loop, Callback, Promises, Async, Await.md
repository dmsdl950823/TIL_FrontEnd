...
<sub>비동기적 Web API를 사용하지 않는</sub> 일반적인 JS code 는 동기적으로(한번에 한줄씩, 순서대로) 코드를 실행합니다. 
...
``` js
  ...
  third()    // 1 2 3
```
이 코드는 세개의 함수를 순서대로 실행합니다.

비동기적 Web API가 사용될 경우, 이 규칙은 조금더 복잡해집니다. `setTimeout` 은 비동기적으로 실행될 필요가 있는데, 그렇지 않으면 **전체 브라우저는 해당 코드의 응답을 기다리는 동안 동작하지 않습니다**.  (일반적으로 테스트 해볼 수 있는 내장 API 는 `setTimeout` 입니다)
...
가장 유용하고 자주 쓰는, promise를 return 하는 Web APIs 는 Fetch API 인데, 비동기 자원을 네트워크를 통해 비동기식 리소스 요청을 할 수 있습니다. `fetch` 는 두가지 프로세스를 가지므로, `then` 체이닝 메서드를 필요로 합니다.

``` js
  // github API fetching example
...
`fetch` request는 URL에 요청을 보내어 응답을 비동기적으로 기다립니다. 첫번째 `then` 은 JSON data로 이루어진 응답(`response`)을 익명함수에 전달하고, JSON 을 두번째 `then` 에 전달해주어 console에 데이터를 보여줍니다. `catch` 문은 error 가 발생하면 console에 보여줍니다.

비동기 처리를 위하여 `fetch` API 를 사용하는것은 매우 좋은 방법이지만, 비동기 action을 핸들링 하기 위해서 `then` 을 사용하는 것은 콜백지옥보다는 쉽지만, 아직도 몇몇 개발자들은 동기적으로 코드를 작성하는 방식을 더 선호합니다. 이 요구에 맞추어, ECMAScript 2016(ES7) 은 promise를 다루기 위하여 `async` function과 , `await` keyword 를 도입했습니다.


# Async Function with `async` / `await`


*`async` 함수* 는 비동기적 코드를 동기적인 방식으로 핸들링할 수 있게 해줍니다. `async` 함수는 promise 를 사용하지만, 더 전통적인 JS 문법을 사용합니다.
...
기존과 비슷한 문법이지만 다르게 동작합니다. 이 함수를 실행하면, `[[PromiseStatus]]`, `[[PromiseValue]]` 를 가지는  promise 를 return 합니다.
...
이것은 `async` 함수를 `then` 을 이용하여 값을 핸들링 할 수 있다는 것을 의미합니다. 

``` js 
  getUser().then(response => console.log(response))
```

`async` 함수는 `await` operator를 이용하여 내부에서 promise를 핸들링할 수 있습니다. `await` 는 `async` 함수 내부에서 사용될 수 있고, 해당 코드를 실행하기 전에, promise 가 해결될 때까지 기다립니다.
...
`await` operator가 해결되기 전까지 `data` 가 로그에 표시되지 않습니다. 

`data` 는 이제 `then` 없이도 내부에서 핸들링되 수 있습니다.

마지막으로, fullfilled 된 promise를 비동기 함수 내부에서 처리하기 때문에, error를 함수 내부에서 처리할 수 도 있습니다. `then` 과 함께 했던 `catch` 함수 대신에, 예외 처리를 위하여 `try`/`catch` 를 사용합니다.
...
만약 문제가 없다면 `catch` 블럭을 스킵할 것이고, 있다면 console에 에러를 표시해줄것입니다.

모던 JS 비동기 코드는 종종 `async` / `await` 문법을 사용하는데, 어떻게 promise 가 동작하는지 알고있는것은 중요합니다. 특히 promise가 `async` / `await` 에서 는 다룰수 없는, `Promise.all()` 등을 추가적으로 가질 수 있다는 것등을 알아야 합니다.

* `async` / `await` 는 코드의 유동성을 위하여[ promise 로 이루어진 generator](https://www.digitalocean.com/community/tutorials/understanding-generators-in-javascript#asyncawait-with-generators)로 재생산 될 수 있습니다. [JS generators 의 이해](https://www.digitalocean.com/community/tutorials/understanding-generators-in-javascript) 튜토리얼을 참고하세요.


# 결론

Web API 가 종종 data를 비동기적으로 제공하기 때문에, 비동기적으로 결과를 처리하는 방법은 JS 개발자에게 필수적인 부분입니다. 이 포스팅에서 host environment 가 코드를 순서대로 실행하기 위해서 stack과 queue 를 이용하여 어떻게 event loop 를 사용하는지를 배웠습니다. 

브라우저가 pararell event를 처리하는 더 자세한 정보는 Mozilla Developer Network -  [Concurrency model 과 event loop](https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop) 를 읽어보세요. 
