...
에이전트는 **사용자를 위해 HTTP 요청을 만들어주는 클라이언트 프로그램** 입니다. 웹 요청을 만드는 애플리케이션은 뭐든 HTTP 에이전트 입니다. 다양한 종류의 에이전트가 있는데, 예를들어, 자동화된 사용자 에이전트의 경우, 사람의 통제 없이 스스로 웹을 돌아다니며 HTTP 트랜잭션을 일으키고 콘텐츠를 받아오는 경우 (ex. 스파이더, 크롤러, 웹 로봇 등) 가 있습니다. s
